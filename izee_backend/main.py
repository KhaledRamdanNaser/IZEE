import sys
if "routing_integration" in sys.modules:
    import importlib
    importlib.reload(sys.modules["routing_integration"])
...
from api.routes.trips_workflow import router as trips_router
...
from routing_integration import preload_raptor_indexes_if_cached, start_walking_transfer_preload_background

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Seed default data on startup so all endpoints work from the first request
    db = SessionLocal()
    try:
        print("STARTUP: Running ensure_defaults to seed DB...")
        ensure_defaults(db)
        print("STARTUP: ensure_defaults complete.")
        
        print("STARTUP: Preloading RAPTOR indexes and caches...")
        preload_raptor_indexes_if_cached()
        start_walking_transfer_preload_background()
        print("STARTUP: Routing caches initialized.")
    finally:
        db.close()
    yield
...
# Health‑check endpoint – returns 200 so test scripts can verify the server is alive
...
# Mount Control Center Dashboard static files
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
dashboard_dir = os.path.join(parent_dir, "control_center_dashboard")
if os.path.exists(dashboard_dir):
    app.mount("/control-center-dashboard", StaticFiles(directory=dashboard_dir, html=True), name="control-center-dashboard")
    print(f"Mounted control-center-dashboard from {dashboard_dir}")

# ── Routing / RAPTOR integration ──────────────────────────────────────────────
# Registers all geocoding, trip-plan, and debug routing endpoints onto this app.
# The original routing block in the docstring below is left untouched.
from routing_integration import register_routing_routes  # noqa: E402
...
_cache_modes = []
if EXPERIMENTAL_BRT_ENABLED:
cache_modes.append("brt")
if EXPERIMENTAL_LRT_ENABLED:
cache_modes.append("lrt")
...
# ── Transparent Routing Delegation Proxy ──────────────────────────────────
# This proxy redirects access/modification of routing variables, schemas,
# caches, and helper functions to `routing_integration.py` to ensure complete
# visibility and seamless synchronization (e.g. for testing processes).
import sys
from types import ModuleType

_DELEGATED_ROUTING_ATTRIBUTES = {
    "GTFS_PATHS",
    "EXPERIMENTAL_BRT_GTFS_PATH",
    "EXPERIMENTAL_LRT_GTFS_PATH",
    "EXPERIMENTAL_MONORAIL_GTFS_PATH",
    "EXPERIMENTAL_BRT_ENABLED",
    "EXPERIMENTAL_LRT_ENABLED",
    "EXPERIMENTAL_MONORAIL_ENABLED",
    "RAPTOR_CACHE_VERSION",
    "RAPTOR_CACHE_PATH",
    "raptor_indexes_cache",
    "walking_transfers_cache",
    "brt_feeder_transfer_count_cache",
    "lrt_feeder_transfer_count_cache",
    "monorail_feeder_transfer_count_cache",
    "eta_segment_requests_cache",
    "geocode_search_cache",
    "walking_transfers_preload_started",
    "TripPlanRequest",
    "Coordinate",
    "get_db",
    "internal_stop_search",
    "merged_place_search",
    "search_places_with_nominatim",
    "get_raptor_indexes",
    "build_brt_feeder_transfers",
    "build_lrt_feeder_transfers",
    "build_monorail_feeder_transfers",
    "trip_plan",
    "trip_plan_stop_to_stop",
}

# Define these in the module namespace so that unittest.mock (e.g. patch.object)
# sees them as local attributes and does not fail on cleanup/restoration.
GTFS_PATHS = None
EXPERIMENTAL_BRT_GTFS_PATH = None
EXPERIMENTAL_LRT_GTFS_PATH = None
EXPERIMENTAL_MONORAIL_GTFS_PATH = None
EXPERIMENTAL_BRT_ENABLED = None
EXPERIMENTAL_LRT_ENABLED = None
EXPERIMENTAL_MONORAIL_ENABLED = None
RAPTOR_CACHE_VERSION = None
RAPTOR_CACHE_PATH = None
raptor_indexes_cache = None
walking_transfers_cache = None
brt_feeder_transfer_count_cache = None
lrt_feeder_transfer_count_cache = None
monorail_feeder_transfer_count_cache = None
eta_segment_requests_cache = None
geocode_search_cache = None
walking_transfers_preload_started = None
TripPlanRequest = None
Coordinate = None
get_db = None
internal_stop_search = None
merged_place_search = None
search_places_with_nominatim = None
get_raptor_indexes = None
build_brt_feeder_transfers = None
build_lrt_feeder_transfers = None
build_monorail_feeder_transfers = None
trip_plan = None
trip_plan_stop_to_stop = None

class RoutingModuleProxy(ModuleType):
    def __getattribute__(self, name):
        if name in _DELEGATED_ROUTING_ATTRIBUTES:
            import routing_integration
            return getattr(routing_integration, name)
        return super().__getattribute__(name)

    def __setattr__(self, name, value):
        if name in _DELEGATED_ROUTING_ATTRIBUTES:
            import routing_integration
            setattr(routing_integration, name, value)
        else:
    ...
    def __delattr__(self, name):
        if name in _DELEGATED_ROUTING_ATTRIBUTES:
            import routing_integration
            if hasattr(routing_integration, name):
                delattr(routing_integration, name)
        else:
            super().__delattr__(name)

sys.modules[__name__].__class__ = RoutingModuleProxy